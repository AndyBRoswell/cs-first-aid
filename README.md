...
ABBYY FineReader PDF 在保存时会自动压缩到尽量小，但该软件在未输入序列号时只能保存少量页面（貌似是 100 页？）。笔者购买了这个软件，当时花了一大笔 RMB。但因为需要扫描的书非常多，相比之下，这笔一次性的购买很值得。

然而，需要注意的是：PDF 并不是被设计为方便进行笔记的格式。因而对 PDF 的编辑并不如一些常见格式（如 DOCX）来得方便。当 PDF 较大时，反复编辑与保存会带来相当大的写入量。现如今，各存储厂商为充分压榨利润，
