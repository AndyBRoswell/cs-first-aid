...
import { foreign_locales, get_release_stages, root_language, sidebar_links } from '@tests/util/config/starlight.ts'
...
src_util.test('sidebar badges are complete, and blank for unavailable pages or translations', { tag: '@Sidebar' }, async ({ page }) => {
  ...
  await expect(rendered_sidebar_links).toHaveCount(sidebar_links.length) // Synchronize before taking a positional snapshot of the complete sidebar.
  ...
  await src_util.test.step('unavailable translations render blank release badges in every foreign locale', async () => {
    for (const { locale, language } of foreign_locales) {
      ...
      await expect(localized_sidebar_links).toHaveCount(sidebar_links.length) // Wait for this locale's complete sidebar before taking its snapshot.
      const localized_sidebar_snapshot: Rendered_Sidebar_Link[] = await get_rendered_sidebar_snapshot(localized_sidebar_links)

      for (const [ sidebar_index, item ] of sidebar_links.entries()) {
